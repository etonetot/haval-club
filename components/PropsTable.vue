  ...
  <div id="table-scroll" class="table-scroll" ref="TableScroll">
       ...
       <table class="main-table">
         <tbody ref="TableScrollTBody">
          <tr v-for="(row, ir) in rows" :class="row.class" :key="ir">
            ...
            <td v-for="(col, ic) in colsTBody" v-html="row[ col.key ]" :key="ic">
            </td>
          </tr>
         </tbody>
       </table>
    ...
    <table class="main-table clone">
      ...
      <tr v-for="(row, ir) in rows" :class="row.class" :ref="'tr'+ir" :key="ir">
        <th v-html="row[ cols[0].key ]" class="fixed-side"></th>
        <td v-for="(col, ic) in colsTBody" v-html="row[ col.key ]" :key="ic"></td>
      </tr>
      </tbody>
    </table>

    <template v-for="row in scrollrows">
      <div  class="SecondScroll SecondScrollAdditional" @scroll="OnScrollSecond" :ref="'SecondScroll'+row.ind" :key="row.ind">
        <div class="SecondScrollInner" :ref="'SecondScrollInner'+row.ind"></div>
      </div>
    </template> 
